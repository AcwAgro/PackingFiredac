object FfrmImpEtiqBins: TFfrmImpEtiqBins
  Left = 303
  Top = 223
  BorderIcons = [biSystemMenu]
  Caption = 'Impress'#227'o de etiqueta - Entrada Bins.'
  ClientHeight = 443
  ClientWidth = 744
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object RzPanel1: TRzPanel
    Left = 140
    Top = 0
    Width = 604
    Height = 443
    Align = alClient
    BorderInner = fsLowered
    BorderOuter = fsFlatRounded
    BorderColor = 14273198
    BorderHighlight = clWhite
    BorderShadow = clSilver
    BorderWidth = 3
    Color = 15987699
    GradientColorStyle = gcsCustom
    GradientColorStop = clWhite
    GridXSize = 5
    GridYSize = 5
    TabOrder = 0
    VisualStyle = vsGradient
    object PDados: TPanel
      Left = 7
      Top = 7
      Width = 457
      Height = 429
      Align = alLeft
      BevelOuter = bvNone
      Color = clWhite
      ParentBackground = False
      TabOrder = 0
      object Label7: TLabel
        Left = 8
        Top = 3
        Width = 66
        Height = 13
        Caption = 'Variedade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 105
        Top = 241
        Width = 74
        Height = 13
        Caption = 'Quantidade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
      object Label1: TLabel
        Left = 8
        Top = 43
        Width = 57
        Height = 13
        Caption = 'Produtor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label2: TLabel
        Left = 8
        Top = 83
        Width = 64
        Height = 13
        Caption = 'Romaneio'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 98
        Top = 83
        Width = 30
        Height = 13
        Caption = 'Data'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label4: TLabel
        Left = 8
        Top = 123
        Width = 59
        Height = 13
        Caption = 'Tipo Bins'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 214
        Top = 83
        Width = 72
        Height = 13
        Caption = 'Nota Fiscal'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label8: TLabel
        Left = 8
        Top = 163
        Width = 42
        Height = 13
        Caption = 'Linhas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label9: TLabel
        Left = 347
        Top = 3
        Width = 44
        Height = 13
        Caption = 'C'#243'digo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label10: TLabel
        Left = 347
        Top = 43
        Width = 44
        Height = 13
        Caption = 'C'#243'digo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object SBP2: TSpeedButton
        Tag = 100
        Left = 324
        Top = 16
        Width = 20
        Height = 18
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        NumGlyphs = 2
        Spacing = 0
        OnClick = SBP2Click
      end
      object SBP3: TSpeedButton
        Tag = 100
        Left = 324
        Top = 56
        Width = 20
        Height = 18
        Glyph.Data = {
          
          E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
        NumGlyphs = 2
        Spacing = 0
        OnClick = SBP3Click
      end
      object M1: TMaskEdit
        Left = 8
        Top = 16
        Width = 315
        Height = 21
        CharCase = ecUpperCase
        MaxLength = 120
        TabOrder = 0
        Text = ''
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object edQtde: TSpinEdit
        Left = 187
        Top = 236
        Width = 55
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Verdana'
        Font.Style = []
        MaxLength = 3
        MaxValue = 1000
        MinValue = 1
        ParentFont = False
        TabOrder = 10
        Value = 1
      end
      object edContador: TSpinEdit
        Left = 275
        Top = 236
        Width = 55
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Verdana'
        Font.Style = []
        MaxLength = 3
        MaxValue = 10000
        MinValue = 1
        ParentFont = False
        TabOrder = 11
        Value = 1
        Visible = False
      end
      object M2: TMaskEdit
        Left = 8
        Top = 56
        Width = 315
        Height = 21
        CharCase = ecUpperCase
        MaxLength = 120
        TabOrder = 2
        Text = ''
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M3: TMaskEdit
        Left = 8
        Top = 96
        Width = 81
        Height = 21
        CharCase = ecUpperCase
        EditMask = '999999;1; '
        MaxLength = 6
        TabOrder = 4
        Text = '      '
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M6: TMaskEdit
        Left = 8
        Top = 137
        Width = 249
        Height = 21
        CharCase = ecUpperCase
        MaxLength = 30
        TabOrder = 7
        Text = ''
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M5: TMaskEdit
        Left = 214
        Top = 96
        Width = 85
        Height = 21
        CharCase = ecUpperCase
        EditMask = '999999999;1; '
        MaxLength = 9
        TabOrder = 6
        Text = '         '
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M7: TMaskEdit
        Left = 8
        Top = 177
        Width = 315
        Height = 21
        MaxLength = 30
        TabOrder = 8
        Text = ''
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M8: TMaskEdit
        Left = 8
        Top = 199
        Width = 315
        Height = 21
        MaxLength = 30
        TabOrder = 9
        Text = ''
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
      end
      object M4: TcxDateEdit
        Left = 98
        Top = 96
        AutoSize = False
        Properties.DateButtons = [btnToday]
        Properties.DateOnError = deToday
        Properties.DisplayFormat = 'dd/MM/yyyy'
        Style.BorderStyle = ebsSingle
        TabOrder = 5
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
        Height = 21
        Width = 110
      end
      object M10: TcxCurrencyEdit
        Left = 347
        Top = 56
        Properties.DisplayFormat = '0'
        TabOrder = 3
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
        Width = 70
      end
      object M9: TcxCurrencyEdit
        Left = 347
        Top = 16
        Properties.DisplayFormat = '0'
        TabOrder = 1
        OnKeyDown = FormKeyDown
        OnKeyPress = FormKeyPress
        Width = 70
      end
    end
  end
  object rzpUtilitario: TRzPanel
    Left = 0
    Top = 0
    Width = 140
    Height = 443
    Align = alLeft
    BorderInner = fsLowered
    BorderOuter = fsFlatRounded
    BorderColor = 14273198
    BorderHighlight = clWhite
    BorderShadow = clSilver
    BorderWidth = 3
    Color = 15987699
    GradientColorStyle = gcsCustom
    GradientColorStop = clWhite
    GridXSize = 5
    GridYSize = 5
    TabOrder = 1
    VisualStyle = vsGradient
    object sb7: TSpeedButton
      Tag = 1
      Left = 7
      Top = 23
      Width = 126
      Height = 54
      Hint = 'Imprimir registros...'
      Align = alTop
      Caption = 'Imprimir'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      Glyph.Data = {
        
        0000}
      Layout = blGlyphTop
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      Spacing = 0
      OnClick = sb7Click
      ExplicitLeft = 32
      ExplicitTop = 49
      ExplicitWidth = 72
    end
    object RzPanel3: TRzPanel
      Left = 7
      Top = 7
      Width = 126
      Height = 16
      Align = alTop
      Alignment = taLeftJustify
      AutoSize = True
      BorderOuter = fsNone
      Color = 15987699
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Verdana'
      Font.Style = []
      GradientColorStyle = gcsCustom
      GradientColorStop = clWhite
      GradientDirection = gdHorizontalCenter
      ParentFont = False
      TabOrder = 0
      VisualStyle = vsGradient
      object RzPanel5: TRzPanel
        Left = 0
        Top = 0
        Width = 126
        Height = 16
        Align = alTop
        BorderOuter = fsFlatRounded
        BorderSides = [sdTop]
        Caption = 'Utilit'#225'rio'
        Color = 15987699
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 5460819
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        GradientColorStyle = gcsCustom
        GradientColorStart = 10611152
        GradientColorStop = 10611152
        GradientDirection = gdVerticalEnd
        ParentFont = False
        TabOrder = 0
        VisualStyle = vsGradient
      end
    end
  end
  object sqlDados: TQuery
    DatabaseName = 'JaeAdmin'
    SessionName = 'Sesao1'
    SQL.Strings = (
      'Select * From etiqBins')
    Left = 282
    Top = 134
    object sqlDadosVARIEDADE: TStringField
      FieldName = 'VARIEDADE'
      Origin = 'JAEADMIN.ETIQBINS.VARIEDADE'
      Size = 50
    end
    object sqlDadosPRODUTOR: TStringField
      FieldName = 'PRODUTOR'
      Origin = 'JAEADMIN.ETIQBINS.PRODUTOR'
      Size = 50
    end
    object sqlDadosROMANEIO: TStringField
      FieldName = 'ROMANEIO'
      Origin = 'JAEADMIN.ETIQBINS.ROMANEIO'
      Size = 6
    end
    object sqlDadosDATA: TStringField
      FieldName = 'DATA'
      Origin = 'JAEADMIN.ETIQBINS.DATA'
      Size = 10
    end
    object sqlDadosTIPOBINS: TStringField
      FieldName = 'TIPOBINS'
      Origin = 'JAEADMIN.ETIQBINS.TIPOBINS'
      Size = 50
    end
    object sqlDadosNOTA: TStringField
      FieldName = 'NOTA'
      Origin = 'JAEADMIN.ETIQBINS.NOTA'
      Size = 9
    end
    object sqlDadosMOTORISTA: TStringField
      FieldName = 'MOTORISTA'
      Origin = 'JAEADMIN.ETIQBINS.MOTORISTA'
      Size = 50
    end
    object sqlDadosPESOKG: TStringField
      FieldName = 'PESOKG'
      Origin = 'JAEADMIN.ETIQBINS.PESOKG'
      Size = 10
    end
    object sqlDadosPESOBRUTO: TStringField
      FieldName = 'PESOBRUTO'
      Origin = 'JAEADMIN.ETIQBINS.PESOBRUTO'
      Size = 10
    end
    object sqlDadosPESOTARA: TStringField
      FieldName = 'PESOTARA'
      Origin = 'JAEADMIN.ETIQBINS.PESOTARA'
      Size = 10
    end
    object sqlDadosPESOLIQUIDO: TStringField
      FieldName = 'PESOLIQUIDO'
      Origin = 'JAEADMIN.ETIQBINS.PESOLIQUIDO'
      Size = 10
    end
    object sqlDadosLINHA1: TStringField
      FieldName = 'LINHA1'
      Origin = 'JAEADMIN.ETIQBINS.LINHA1'
      Size = 30
    end
    object sqlDadosLINHA2: TStringField
      FieldName = 'LINHA2'
      Origin = 'JAEADMIN.ETIQBINS.LINHA2'
      Size = 30
    end
    object sqlDadosLINHA3: TStringField
      FieldName = 'LINHA3'
      Origin = 'JAEADMIN.ETIQBINS.LINHA3'
      Size = 30
    end
    object sqlDadosLINHA4: TStringField
      FieldName = 'LINHA4'
      Origin = 'JAEADMIN.ETIQBINS.LINHA4'
      Size = 30
    end
    object sqlDadosLINHA5: TStringField
      FieldName = 'LINHA5'
      Origin = 'JAEADMIN.ETIQBINS.LINHA5'
      Size = 30
    end
    object sqlDadosQTDE: TIntegerField
      FieldName = 'QTDE'
      Origin = 'JAEADMIN.ETIQBINS.QTDE'
    end
    object sqlDadosCONTADOR: TIntegerField
      FieldName = 'CONTADOR'
      Origin = 'JAEADMIN.ETIQBINS.CONTADOR'
    end
    object sqlDadosBARRA: TStringField
      FieldName = 'BARRA'
      Origin = 'JAEADMIN.ETIQBINS.BARRA'
      Size = 12
    end
    object sqlDadosVARCOD: TIntegerField
      FieldName = 'VARCOD'
      Origin = 'JAEADMIN.ETIQBINS.VARCOD'
    end
    object sqlDadosPRODCOD: TIntegerField
      FieldName = 'PRODCOD'
      Origin = 'JAEADMIN.ETIQBINS.PRODCOD'
    end
  end
  object dspDados: TDataSetProvider
    DataSet = sqlDados
    UpdateMode = upWhereKeyOnly
    Left = 314
    Top = 134
  end
  object cdsDados: TClientDataSet
    Aggregates = <>
    PacketRecords = 30
    Params = <>
    ProviderName = 'dspDados'
    Left = 346
    Top = 134
    object cdsDadosVARIEDADE: TStringField
      FieldName = 'VARIEDADE'
      Size = 50
    end
    object cdsDadosPRODUTOR: TStringField
      FieldName = 'PRODUTOR'
      Size = 50
    end
    object cdsDadosROMANEIO: TStringField
      FieldName = 'ROMANEIO'
      Size = 6
    end
    object cdsDadosDATA: TStringField
      FieldName = 'DATA'
      Size = 10
    end
    object cdsDadosTIPOBINS: TStringField
      FieldName = 'TIPOBINS'
      Size = 50
    end
    object cdsDadosNOTA: TStringField
      FieldName = 'NOTA'
      Size = 9
    end
    object cdsDadosMOTORISTA: TStringField
      FieldName = 'MOTORISTA'
      Size = 50
    end
    object cdsDadosPESOKG: TStringField
      FieldName = 'PESOKG'
      Size = 10
    end
    object cdsDadosPESOBRUTO: TStringField
      FieldName = 'PESOBRUTO'
      Size = 10
    end
    object cdsDadosPESOTARA: TStringField
      FieldName = 'PESOTARA'
      Size = 10
    end
    object cdsDadosPESOLIQUIDO: TStringField
      FieldName = 'PESOLIQUIDO'
      Size = 10
    end
    object cdsDadosLINHA1: TStringField
      FieldName = 'LINHA1'
      Size = 30
    end
    object cdsDadosLINHA2: TStringField
      FieldName = 'LINHA2'
      Size = 30
    end
    object cdsDadosLINHA3: TStringField
      FieldName = 'LINHA3'
      Size = 30
    end
    object cdsDadosLINHA4: TStringField
      FieldName = 'LINHA4'
      Size = 30
    end
    object cdsDadosLINHA5: TStringField
      FieldName = 'LINHA5'
      Size = 30
    end
    object cdsDadosQTDE: TIntegerField
      FieldName = 'QTDE'
    end
    object cdsDadosCONTADOR: TIntegerField
      FieldName = 'CONTADOR'
    end
    object cdsDadosBARRA: TStringField
      FieldName = 'BARRA'
      Size = 12
    end
    object cdsDadosVARCOD: TIntegerField
      FieldName = 'VARCOD'
    end
    object cdsDadosPRODCOD: TIntegerField
      FieldName = 'PRODCOD'
    end
  end
  object dsDados: TDataSource
    DataSet = cdsDados
    Left = 378
    Top = 134
  end
end
