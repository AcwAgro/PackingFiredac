object FfrmCadDivSaida: TFfrmCadDivSaida
  Left = 303
  Top = 223
  BorderIcons = [biSystemMenu]
  Caption = 'Cadastro do Estrutura/Mercadoria.'
  ClientHeight = 463
  ClientWidth = 1070
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object RzPanel1: TRzPanel
    Left = 0
    Top = 0
    Width = 1070
    Height = 463
    Align = alClient
    BorderInner = fsLowered
    BorderOuter = fsFlatRounded
    BorderColor = 14273198
    BorderHighlight = clWhite
    BorderShadow = clSilver
    BorderWidth = 3
    Color = 15987699
    GradientColorStyle = gcsCustom
    GradientColorStop = clWhite
    GridXSize = 5
    GridYSize = 5
    TabOrder = 0
    VisualStyle = vsGradient
    object PDados: TPanel
      Left = 7
      Top = 7
      Width = 1056
      Height = 449
      Align = alClient
      BevelOuter = bvNone
      Color = clWhite
      ParentBackground = False
      TabOrder = 0
      object Label8: TLabel
        Left = 80
        Top = 44
        Width = 4
        Height = 13
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
      object GroupBox7: TGroupBox
        Left = 510
        Top = 0
        Width = 538
        Height = 402
        Align = alLeft
        Caption = ' Lista '
        TabOrder = 1
        object dbDetalhe: TDBGrid
          Left = 2
          Top = 15
          Width = 534
          Height = 385
          Hint = 'Bot'#227'o da direita - Excluir individual...'
          Align = alClient
          Color = clWhite
          DataSource = dsEstru
          FixedColor = 13878195
          ParentShowHint = False
          PopupMenu = PopupMenu1
          ReadOnly = True
          ShowHint = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Verdana'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'QTDE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Verdana'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Qtde.'
              Width = 35
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CODIGO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Verdana'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'C'#243'digo'
              Width = 63
              Visible = True
            end
            item
              Color = 15329769
              Expanded = False
              FieldName = 'DESCRICAO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'Verdana'
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'Produto'
              Width = 360
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'UNIDADE'
              Title.Alignment = taCenter
              Title.Caption = 'Und.'
              Width = 35
              Visible = True
            end>
        end
      end
      object Panel1: TPanel
        Left = 0
        Top = 402
        Width = 1056
        Height = 47
        Align = alBottom
        Color = 15000804
        ParentBackground = False
        TabOrder = 2
        object sbExcluir: TSpeedButton
          Tag = 1
          Left = 121
          Top = 1
          Width = 120
          Height = 45
          Hint = 'Excluir...'
          Align = alLeft
          Caption = '  Excluir'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Glyph.Data = {
            
            C9DED2CCDDD3CCDBCFC9D8CFC6DFD5CBF9E1D5F9E1D5F9E1D500}
          Margin = 4
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          Spacing = 5
          OnClick = SB4Click
          ExplicitLeft = 161
          ExplicitTop = 0
        end
        object SB4: TSpeedButton
          Left = 1
          Top = 1
          Width = 120
          Height = 45
          Hint = 'Cadastrar...'
          Align = alLeft
          Caption = ' Cadastrar'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Glyph.Data = {
            
            D5EBD6D4ECD6D0EAD7CFECDCD5EFE7E0FCE4D2FCE4D2FCE4D200}
          Margin = 4
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          Spacing = 5
          OnClick = SB4Click
          ExplicitLeft = -5
          ExplicitTop = 6
        end
        object sbAtualizar: TSpeedButton
          Tag = 13
          Left = 512
          Top = 1
          Width = 120
          Height = 45
          Align = alLeft
          Caption = ' Atualizar'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Glyph.Data = {
            
            0000}
          Margin = 4
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          Spacing = 5
          OnClick = sbAtualizarClick
          ExplicitLeft = 693
          ExplicitTop = -7
        end
        object sbExcluirLista: TSpeedButton
          Tag = 1
          Left = 632
          Top = 1
          Width = 130
          Height = 45
          Hint = 'Excluir...'
          Align = alLeft
          Caption = '  Excluir Lista'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Glyph.Data = {
            
            C9DED2CCDDD3CCDBCFC9D8CFC6DFD5CBF9E1D5F9E1D5F9E1D500}
          Margin = 4
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          Spacing = 5
          OnClick = sbExcluirListaClick
          ExplicitLeft = 914
          ExplicitTop = 4
        end
        object Panel3: TPanel
          Left = 241
          Top = 1
          Width = 271
          Height = 45
          Align = alLeft
          BevelOuter = bvNone
          TabOrder = 0
        end
      end
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 510
        Height = 402
        Align = alLeft
        BevelOuter = bvNone
        TabOrder = 0
        object Panel4: TPanel
          Left = 0
          Top = 0
          Width = 510
          Height = 161
          Align = alTop
          TabOrder = 0
          object Label7: TLabel
            Left = 12
            Top = 7
            Width = 50
            Height = 16
            Caption = 'C'#243'digo'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object SBP1: TSpeedButton
            Tag = 100
            Left = 68
            Top = 25
            Width = 24
            Height = 24
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = []
            Glyph.Data = {
              
              E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
            NumGlyphs = 2
            ParentFont = False
            Spacing = 0
            OnClick = SBP1Click
          end
          object Label11: TLabel
            Left = 92
            Top = 7
            Width = 81
            Height = 16
            Caption = 'Mercadoria'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label2: TLabel
            Left = 12
            Top = 49
            Width = 40
            Height = 16
            Caption = 'Qtde.'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = [fsBold]
            ParentFont = False
            Transparent = True
          end
          object Label6: TLabel
            Left = 12
            Top = 94
            Width = 74
            Height = 16
            Caption = 'Variedade'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = [fsBold]
            ParentFont = False
            Transparent = True
          end
          object SBP2: TSpeedButton
            Tag = 100
            Left = 51
            Top = 110
            Width = 24
            Height = 24
            Glyph.Data = {
              
              E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
            NumGlyphs = 2
            Spacing = 0
            OnClick = SBP2Click
          end
          object Label1: TLabel
            Left = 12
            Top = 137
            Width = 178
            Height = 13
            Caption = '*Obs: 00-Todas Variedades'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clMaroon
            Font.Height = -11
            Font.Name = 'Verdana'
            Font.Style = [fsBold]
            ParentFont = False
            Transparent = True
          end
          object ME1: TcxCurrencyEdit
            Left = 12
            Top = 25
            ParentFont = False
            Properties.DisplayFormat = '0'
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clWindowText
            Style.Font.Height = -13
            Style.Font.Name = 'Verdana'
            Style.Font.Style = []
            Style.IsFontAssigned = True
            TabOrder = 0
            OnExit = ME1Exit
            OnKeyDown = FormKeyDown
            OnKeyPress = FormKeyPress
            Width = 56
          end
          object Edit1: TEdit
            Left = 93
            Top = 25
            Width = 321
            Height = 24
            CharCase = ecUpperCase
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            OnKeyDown = FormKeyDown
            OnKeyPress = FormKeyPress
          end
          object ME2: TcxCurrencyEdit
            Left = 12
            Top = 110
            ParentFont = False
            Properties.DisplayFormat = '0'
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clWindowText
            Style.Font.Height = -13
            Style.Font.Name = 'Verdana'
            Style.Font.Style = []
            Style.IsFontAssigned = True
            TabOrder = 3
            OnExit = ME2Exit
            OnKeyDown = FormKeyDown
            OnKeyPress = FormKeyPress
            Width = 38
          end
          object Edit2: TEdit
            Left = 76
            Top = 110
            Width = 256
            Height = 24
            CharCase = ecUpperCase
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'Verdana'
            Font.Style = []
            ParentFont = False
            TabOrder = 4
            Text = 'TODAS AS VARIEDADES'
            OnKeyDown = FormKeyDown
            OnKeyPress = FormKeyPress
          end
          object rzQtde: TcxCurrencyEdit
            Left = 12
            Top = 67
            EditValue = 0.000000000000000000
            ParentFont = False
            Properties.DecimalPlaces = 3
            Properties.DisplayFormat = '0.###'
            Style.Font.Charset = DEFAULT_CHARSET
            Style.Font.Color = clWindowText
            Style.Font.Height = -13
            Style.Font.Name = 'Verdana'
            Style.Font.Style = []
            Style.IsFontAssigned = True
            TabOrder = 2
            OnExit = ME1Exit
            OnKeyDown = FormKeyDown
            OnKeyPress = FormKeyPress
            Width = 61
          end
        end
        object Panel5: TPanel
          Left = 0
          Top = 161
          Width = 510
          Height = 241
          Align = alClient
          TabOrder = 1
          object Panel6: TPanel
            Left = 1
            Top = 1
            Width = 375
            Height = 239
            Align = alClient
            TabOrder = 0
            object Label4: TLabel
              Left = 1
              Top = 1
              Width = 373
              Height = 16
              Align = alTop
              Caption = 'Categoria'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Verdana'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = True
              ExplicitWidth = 71
            end
            object clbCat: TCheckListBox
              Left = 1
              Top = 17
              Width = 373
              Height = 221
              Align = alClient
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -16
              Font.Name = 'Verdana'
              Font.Style = []
              ItemHeight = 18
              Items.Strings = (
                'a'
                'b'
                'c')
              ParentFont = False
              TabOrder = 0
            end
          end
          object Panel7: TPanel
            Left = 376
            Top = 1
            Width = 133
            Height = 239
            Align = alRight
            TabOrder = 1
            object Label5: TLabel
              Left = 1
              Top = 1
              Width = 131
              Height = 16
              Align = alTop
              Caption = 'Calibre/Tamanho'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Verdana'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = True
              ExplicitWidth = 127
            end
            object clbTam: TCheckListBox
              Left = 1
              Top = 17
              Width = 131
              Height = 221
              Align = alClient
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -16
              Font.Name = 'Verdana'
              Font.Style = []
              ItemHeight = 18
              Items.Strings = (
                'a'
                'b'
                'c')
              ParentFont = False
              TabOrder = 0
            end
          end
        end
      end
    end
  end
  object ApplicationEvents1: TApplicationEvents
    OnMessage = ApplicationEvents1Message
    Left = 400
    Top = 56
  end
  object dsEstru: TDataSource
    DataSet = sqlEstru
    Left = 898
    Top = 110
  end
  object PopupMenu1: TPopupMenu
    Left = 928
    Top = 200
    object Excluir1: TMenuItem
      Caption = 'Excluir'
      ShortCut = 16430
      OnClick = Excluir1Click
    end
  end
  object sqlEstru: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    SQL.Strings = (
      'select cadbx.qtde, cadpr.codigo, cadpr.descricao, cadpr.unidade'
      'from cadprobx cadbx'
      'inner join cadprodu cadpr on cadpr.codigo = cadbx.lanc'
      'where cadbx.codigo = :pcod '
      'order by cadpr.codigo')
    Left = 850
    Top = 110
    ParamData = <
      item
        Name = 'PCOD'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object sqlEstruQTDE: TBCDField
      FieldName = 'QTDE'
      Origin = 'QTDE'
      Precision = 18
    end
    object sqlEstruCODIGO: TIntegerField
      AutoGenerateValue = arDefault
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = []
    end
    object sqlEstruDESCRICAO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'DESCRICAO'
      Origin = 'DESCRICAO'
      ProviderFlags = []
      Size = 50
    end
    object sqlEstruUNIDADE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'UNIDADE'
      Origin = 'UNIDADE'
      ProviderFlags = []
      Size = 2
    end
  end
end
