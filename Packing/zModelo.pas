...
begin
  ...
  begin
    ...
    //Foca em dados
    try dbDados.SetFocus; except end;
  end;
...
begin
  ...
  try
    ...
    if ((dsDados.DataSet As TClientDataSet).IndexName <> 'iIndice') then
    begin
    ...
    dbDados.Canvas.Font.Color:= clRed;
{
   begin
      ...
      begin
        for vi := 0 to dbDados.SelectedRows.Count-1 do
        begin
          GotoBookmarkdbDados.SelectedRows.Items[vi]));
          xvfilt := xvfilt + FieldByName(vcampo).asString + ', ';
        end;
      end;
}
...
procedure TFModelo.cdsDadosAfterApplyUpdates(Sender: TObject;
  var OwnerData: OleVariant);
var vCampo: String;
