...
var vconf, varr, vestx, vestarq:String;
    vArq:TextFile;
begin
  if cdsDadosFecha.AsString = 'S' then
  begin
  ...
  vestx := IntToStr(TComponent(Sender).Tag);
  if (vestx = '1') then
    vestarq := ''
  else
    vestarq := vestx;

  if frPerg(PChar('Deseja Abrir ESTEIRA -> '+ vestx +#13+
                  'Lote nº '+cdsDadosLote.asString)) then
  begin
    try
      vconf := xdirSi+'DADOS\Config'+vestarq+'.INI';
      DeleteFile(vconf);
      AssignFile(varq,xdirsi+'DADOS\Config'+vestarq+'.INI');
      Rewrite(varq);
      ...
      if fileexists(xdirsi+'DADOS\MUDOU'+vestarq+'.INI') then
      begin
        DeleteFile(xdirini+'DADOS\MUDOU'+vestarq+'.INI');
        Sleep(300);
      end;

      AssignFile(varq,xdirsi+'DADOS\MUDOU'+vestarq+'.INI');
      Rewrite(varq);
      Writeln(varq,'Lote     ='+cdsDadosLote.Text);
      CloseFile(varq);
...
procedure TFfrmRomanAbre.tsDadosShow(Sender: TObject);
begin
  inherited;
