...
var
  ...
  vxclres,vxclger,vxcodig:String;
...
procedure TqrClassCli.QuickRepPreview(Sender: TObject);
begin
  fPreview(Sender);
end;

procedure TqrClassCli.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrVenda.Close;
  if vxclger='N' then begin
     ...
     //qrVenda.Sql.Strings[2]:=Copy(qrProdu.Sql.Strings[2],1,189)+'      and pp.Codigo='+qrProdu.FieldByName('Codigo').Text;
...
procedure TqrClassCli.QRSubDetail2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
procedure TqrClassCli.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TqrClassCli.fSomaCat;
begin
  vsoma:=0;
  vxcodig:=frPree(qrVenda.Fields[0].asInteger,7,'0',0);
  while vsoma=0 do begin
    if Copy(vxcodig,1,4)=Copy(frPree(qrVenda.FieldByName('Codigo').Text,7,'0',0),1,4) then
       vqtc:=vqtc+qrVenda.FieldByName('ge').asFloat
    else vsoma:=1;
    qrVenda.Next;
    if qrVenda.Eof then vsoma:=1;
  end;
  qrVenda.Locate('Codigo',vxcodig,[]);
  vsoma:=0;
end;

procedure TqrClassCli.fSomaVar;
begin
  vsoma:=0;
  vxcodig:=frPree(qrVenda.Fields[0].asInteger,7,'0',0);
  while vsoma=0 do begin
    if Copy(vxcodig,1,2)=Copy(frPree(qrVenda.FieldByName('Codigo').Text,7,'0',0),1,2) then
