...
procedure TqrResGru.QuickRepPreview(Sender: TObject);
begin
  vpr:=0;
  fPreview(Sender);
end;

procedure TqrResGru.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vpr:=0;
end;

procedure TqrResGru.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ...
  vrec:=0;
  qrAmos.Close;
  qrAmos.Sql.Strings[2]:='Where (cc.Codigo=am.VarieCod) and (am.Lanc=cm.Lanc) and am.ProdCod='+
                qrMovim.FieldByName('ProdCod').Text+' and cc.GrupoCod='+
                qrMovim.FieldByName('GrupoCod').Text;
  qrAmos.Open;
...
procedure TqrResGru.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  fSql(1,1,['Select Sum(cm.Peso) as vPeso From CadVarie cc, Amostra am, AmostCom cm',
            'Where (cc.Codigo=am.VarieCod) and (am.Lanc=cm.Lanc) and  cc.GrupoCod='+
                    qrMovim.FieldByName('GrupoCod').Text+' and am.ProdCod='+
