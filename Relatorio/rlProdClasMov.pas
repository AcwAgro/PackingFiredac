...
type
  TqrProdClasMov = class(TQuickRep)
    QRBand1: TQRBand;
...
var
  qrProdClasMov: TqrProdClasMov;
  vqt1,vqt3,vqt11,vqt33,vtqt1,vtqt3,vcqt1,vcqt3:Double;
  vqtc,vqtv:Real;
  vsoma,vclo,vcat,vgru,vvar,vcatdes:Integer;
  vlcger,vcodig:String;
...
procedure TqrProdClasMov.QuickRepPreview(Sender: TObject);
begin
...
procedure TqrProdClasMov.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
...
procedure TqrProdClasMov.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  qrVenda.Close;
  if vlcger='N' then begin
...
procedure TqrProdClasMov.QRSubDetail2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TqrProdClasMov.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ...
  if vvar<>StrToInt(Copy(qrVenda.FieldByname('ProdCod').Text,1,2)) then begin
     ...
     qrLabel72.Caption := 'Qtde.: '+FormatFloat('#,0.###', StrToFloatDef(fBus(1,['Select sum(pp.qtdebin) FROM roman rr, romanpro pp '+
                                                                                   'Where (rr.lanc = pp.lanc) ' +vprodutor+' and rr.VarieCod = '+IntToStr(vvar)+vpcmLoc],1),0));
  ...
  if vclo<>StrToInt(Copy(qrVenda.FieldByname('ProdCod').Text,1,1)) then begin
     ...
     qrLabel71.Caption := 'Qtde.: '+FormatFloat('#,0.###', StrToFloatDef(fBus(1,['Select sum(pp.qtdebin) FROM roman rr, romanpro pp '+
                                                                                  'Where (rr.lanc = pp.lanc) ' +vprodutor+' and SubString(rr.VarieCod from 1 for 1) = '+IntToStr(vclo)+vpcmLoc],1),0));
...
procedure TqrProdClasMov.fSomaCat;
begin
  vsoma:=0;
...
procedure TqrProdClasMov.fSomaVar;
begin
  vsoma:=0;
  vcodig:=frPree(qrVenda.Fields[0].asInteger,7,'0',0);
  while vsoma=0 do begin
