...
type
  TqrResDefGerSem = class(TQuickRep)
...
var
  qrResDefGerSem: TqrResDefGerSem;
  vrec,vpr:Integer;
...
procedure TqrResDefGerSem.QuickRepPreview(Sender: TObject);
begin
  vpr:=0; vpes:=0; vqtd:=0;
  fPreview(Sender);
end;

procedure TqrResDefGerSem.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vpr:=0; vpes:=0; vqtd:=0;
end;

procedure TqrResDefGerSem.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  vpes:=vpes+vliq;
  ...
  vrec:=0;
  qrAmos.Close;
  qrAmos.Sql.Strings[2]:='Where (am.Lanc=cm.Lanc) and  am.VarCod='+
...
procedure TqrResDefGerSem.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
