...
procedure TqrResAmoGer.QuickRepPreview(Sender: TObject);
begin
  vpr:=0; vpes:=0; vqtd:=0;
  fPreview(Sender);
end;

procedure TqrResAmoGer.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vpr:=0; vpes:=0; vqtd:=0;
end;

procedure TqrResAmoGer.DetailBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ...
  if qrAmos.RecordCount<1 then begin
     ...
     qrLabel1.Caption:=FloatToStrF(vpes,ffNumber,13,0);

  end;
end;

procedure TqrResAmoGer.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  fSql(1,1,['Select Sum(cm.Peso) as vPeso From Amostra am, AmostCom cm',
            'Where (am.Lanc=cm.Lanc) and  am.VarieCod='+
                    qrMovim.FieldByName('VarieCod').Text],2);
