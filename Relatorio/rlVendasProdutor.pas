...
type
  TqrVendasProdutor = class(TQuickRep)
    QRBand1: TQRBand;
  ...
  private
    ...
    //procedure fSomaVar;
  public
    vDataPervp, vDemaisvp: String;
  end;

var
  qrVendasProdutor: TqrVendasProdutor;
  ...
  vclresvp,vclgervp,vcodigvp,vprodutorvp,vcrlocvp,vcrlocdesvp:String;
  vcod, vcod1, vcod2, vcod3, vcod4, vcod5, vcod6, vcod7: Integer;
...
procedure TqrVendasProdutor.QuickRepPreview(Sender: TObject);
begin
  fPreview(Sender);
end;

procedure TqrVendasProdutor.DetailBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin

  qrVenda.Close;
  qrVenda.Sql.Strings[2]:=Copy(qrProdu.Sql.Strings[2],1,138)+'      and pp.Codigo>0'+vcrlocvp;
  vprodutorvp := ' and pp.Codigo >= 0' + vDataPervp;
  qrVenda.Open;
...
procedure TqrVendasProdutor.QRSubDetail2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
begin
  ...
  begin
    ...
    begin
      ...
      if vcod1 <> 0 then
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod1),
                ' order by 1'],4);
    ...
    begin
      ...
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod2),
                ' order by 1'],4);
    ...
    begin
      ...
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod3),
                ' order by 1'],4);
    ...
    begin
      ...
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod4),
                ' order by 1'],4);
    ...
    begin
      ...
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod5),
                ' order by 1'],4);
    ...
    begin
      ...
      begin
        fSql(1,2,['select SUM(co.QTDE) AS QT',
                '  from esvenda pp, esvendco co',
                ' Where pp.Lanc=co.Lanc '+vDataPervp+' and co.codigo=0'+qrVenda.Fields[0].asString+
                ' and co.prodcod = 0'+IntToStr(vcod6),
                ' order by 1'],4);
...
procedure TqrVendasProdutor.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
