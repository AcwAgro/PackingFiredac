...
var
  qrPDLoteGer: TqrPDLoteGer;
  rtip,rgdata,rgvarie,vpro,vcat,vvar:String;
...
procedure TqrPDLoteGer.QuickRepPreview(Sender: TObject);
begin
  vqtd1:=0; vqtd2:=0; vqtdt1:=0; vqtdt2:=0;
  fPreview(Sender);
end;

procedure TqrPDLoteGer.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  vqtd1:=0; vqtd2:=0; vqtdt1:=0; vqtdt2:=0;
end;

procedure TqrPDLoteGer.PageHeaderBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
...
begin
  ...
  if trim(vpro)<>'' then begin
    fSql(1,1,['Select cc.Lote, Count(cc.Lote), Sum(PesoPro), Sum(Perca), Sum(QtdeBin), Sum(PesoBru) as vQtdeBin, cc.Lanc',
                ...
                'Where (cc.Lanc=ee.Lanc) and (ee.PesoReal<>0) and '+rgvarie+' and '+rgdata,
...
procedure TqrPDLoteGer.DetailBand1AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
