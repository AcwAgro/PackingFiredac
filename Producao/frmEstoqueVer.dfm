object FfrmEstoqueVer: TFfrmEstoqueVer
  Left = 191
  Top = 114
  BorderIcons = [biSystemMenu, biMaximize]
  Caption = ' Estoque Produtos'
  ClientHeight = 482
  ClientWidth = 1184
  Color = 14930883
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 459
    Width = 1184
    Height = 23
    Align = alBottom
    BevelOuter = bvLowered
    Color = clWhite
    Enabled = False
    ParentBackground = False
    TabOrder = 0
    object DBGrid3: TDBGrid
      Left = 1
      Top = 1
      Width = 420
      Height = 21
      Align = alLeft
      BorderStyle = bsNone
      Color = 14668475
      DataSource = dsTotal
      FixedColor = 14930883
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Verdana'
      Font.Style = []
      Options = [dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Verdana'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Width = 20
          Visible = True
        end>
    end
    object DBGrid4: TDBGrid
      Left = 421
      Top = 1
      Width = 762
      Height = 21
      Align = alClient
      BorderStyle = bsNone
      DataSource = dsTotal
      FixedColor = 14930883
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Verdana'
      Font.Style = []
      Options = [dgEditing, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Verdana'
      TitleFont.Style = []
      OnDblClick = dbGrid2DblClick
      Columns = <
        item
          Color = 15329769
          Expanded = False
          FieldName = 'Saldo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Caption = 'Saldo Atual'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clMaroon
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Color = 15329769
          Expanded = False
          FieldName = 'Quilo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Caption = 'Qtde.KG'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Classificacao'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Classifica'#231#227'o'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Esteira'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Ajuste'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Nota'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nota Fiscal'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Pedido'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Pedido/Aber.'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Anterior'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Saldo Ant.'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Width = 20
          Visible = True
        end>
    end
  end
  object RzPanel1: TRzPanel
    Left = 0
    Top = 0
    Width = 1184
    Height = 459
    Align = alClient
    BorderInner = fsLowered
    BorderOuter = fsFlatRounded
    BorderColor = 14273198
    BorderHighlight = clWhite
    BorderShadow = clSilver
    BorderWidth = 3
    Color = 15987699
    GradientColorStyle = gcsCustom
    GradientColorStop = clWhite
    GridXSize = 5
    GridYSize = 5
    TabOrder = 1
    VisualStyle = vsGradient
    object PMenu: TPanel
      Left = 7
      Top = 7
      Width = 1170
      Height = 87
      Align = alTop
      BevelOuter = bvNone
      Color = 15921906
      ParentBackground = False
      TabOrder = 0
      object sbCalcular: TSpeedButton
        Tag = 15
        Left = 759
        Top = 20
        Width = 110
        Height = 36
        Caption = 'Calcular'
        Flat = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Verdana'
        Font.Style = []
        Glyph.Data = {
          
          D5EBD6D4ECD6D0EAD7CFECDCD5EFE7E0FCE4D2FCE4D2FCE4D200}
        Margin = 0
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        Spacing = 5
        OnClick = sbCalcularClick
      end
      object sbImprimir: TSpeedButton
        Tag = 18
        Left = 875
        Top = 19
        Width = 110
        Height = 36
        Caption = 'Imprimir'
        Flat = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Verdana'
        Font.Style = []
        Glyph.Data = {
          
          0000}
        Margin = 0
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        Spacing = 5
        OnClick = SB7Click
      end
      object Label5: TLabel
        Left = 36
        Top = 66
        Width = 103
        Height = 16
        Caption = 'Ler:          min.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
      end
      object GroupBox1: TGroupBox
        Left = 375
        Top = 3
        Width = 372
        Height = 61
        Caption = ' Op'#231#245'es para c'#225'lculo '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        object CCB20: TCheckBox
          Left = 7
          Top = 19
          Width = 128
          Height = 17
          Caption = '(+) Classififica'#231#227'o'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 0
        end
        object CCB30: TCheckBox
          Left = 141
          Top = 37
          Width = 91
          Height = 17
          Caption = '(+/-) Ajuste'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 3
        end
        object CCB40: TCheckBox
          Left = 141
          Top = 19
          Width = 91
          Height = 17
          Caption = '(-) Esteira'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 2
        end
        object CCB60: TCheckBox
          Left = 7
          Top = 37
          Width = 103
          Height = 17
          Caption = '(-) Sa'#237'da/N.F.'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 1
        end
        object CCB80: TCheckBox
          Left = 242
          Top = 19
          Width = 116
          Height = 17
          Caption = 'Anterior Zerado'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 4
        end
        object CCB50: TCheckBox
          Left = 242
          Top = 38
          Width = 124
          Height = 17
          Caption = '(-) Pedido/Aberto'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 5
        end
      end
      object GroupBox2: TGroupBox
        Left = 5
        Top = 3
        Width = 161
        Height = 61
        Caption = ' Per'#237'odo '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        object Label2: TLabel
          Left = 41
          Top = 36
          Width = 14
          Height = 16
          Alignment = taRightJustify
          Caption = #224':'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label3: TLabel
          Left = 31
          Top = 16
          Width = 24
          Height = 16
          Alignment = taRightJustify
          Caption = 'De:'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object dtpInicial: TDateTimePicker
          Left = 61
          Top = 16
          Width = 94
          Height = 21
          Date = 41287.000000000000000000
          Time = 41287.000000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          TabStop = False
          OnChange = dtpInicialChange
        end
        object dtpFinal: TDateTimePicker
          Left = 61
          Top = 36
          Width = 94
          Height = 21
          Date = 41287.000000000000000000
          Time = 41287.000000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = False
          OnChange = dtpFinalChange
        end
      end
      object GroupBox3: TGroupBox
        Left = 172
        Top = 3
        Width = 197
        Height = 61
        Caption = ' Variedade '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        object CB2: TComboBox
          Left = 12
          Top = 17
          Width = 173
          Height = 21
          DropDownCount = 20
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object cbAgrupar: TCheckBox
          Left = 12
          Top = 40
          Width = 149
          Height = 17
          Hint = 'Acumula a quantidade na Fam'#237'lia informada na Variedade.'
          Caption = '+ Agrupar/Fam'#237'lia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 1
          OnClick = cbAgruparClick
        end
      end
      object SETime: TSpinEdit
        Left = 66
        Top = 64
        Width = 43
        Height = 22
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        MaxLength = 2
        MaxValue = 60
        MinValue = 0
        ParentFont = False
        TabOrder = 3
        Value = 0
        OnChange = SETimeChange
      end
      object cbSemMovimento: TCheckBox
        Left = 172
        Top = 66
        Width = 168
        Height = 17
        Caption = 'Mostrar Sem Movimento'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        OnClick = cbSemMovimentoClick
        OnExit = cbSemMovimentoExit
      end
    end
    object dbGrid2: TDBGrid
      Left = 427
      Top = 94
      Width = 750
      Height = 358
      Align = alClient
      BorderStyle = bsNone
      DataSource = dsEstoque
      FixedColor = 14930883
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Verdana'
      Font.Style = []
      Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      ReadOnly = True
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Verdana'
      TitleFont.Style = []
      OnDblClick = dbGrid2DblClick
      Columns = <
        item
          Color = 15329769
          Expanded = False
          FieldName = 'QATU'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Caption = 'Saldo Atual'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clMaroon
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Color = 15329769
          Expanded = False
          FieldName = 'SAI9'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Caption = 'Qtde.KG'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QCLA'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Classifica'#231#227'o'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 84
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QEST'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Esteira'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QAJU'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Ajuste'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QNOT'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Nota Fiscal'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QPED'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Pedido/Aber.'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QANT'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Saldo Ant.'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 76
          Visible = True
        end
        item
          Color = 15329769
          Expanded = False
          FieldName = 'SAI8'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Vlr.M'#233'dio'
          Title.Color = 14668475
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Visible = False
        end
        item
          Color = 15329769
          Expanded = False
          FieldName = 'SAI7'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Vlr.Total'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Visible = False
        end>
    end
    object DBGrid1: TDBGrid
      Left = 7
      Top = 94
      Width = 420
      Height = 358
      Align = alLeft
      BorderStyle = bsNone
      Color = 14668475
      DataSource = dsEstoque
      FixedColor = 14930883
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Verdana'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ParentFont = False
      ReadOnly = True
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Verdana'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'PDES'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 270
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PCOD'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'C'#243'digo'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Unid.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 30
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CALD'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Caption = 'Tam.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clNavy
          Title.Font.Height = -11
          Title.Font.Name = 'Arial'
          Title.Font.Style = [fsBold]
          Width = 35
          Visible = True
        end
        item
          Expanded = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Verdana'
          Font.Style = []
          Width = 20
          Visible = True
        end>
    end
  end
  object dsEstoque: TDataSource
    DataSet = TEstoque
    Left = 736
    Top = 192
  end
  object Timer1: TTimer
    Enabled = False
    Interval = 500
    OnTimer = Timer1Timer
    Left = 520
    Top = 128
  end
  object cdsTotal: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 768
    Top = 368
    object cdsTotalSaldo: TFloatField
      FieldName = 'Saldo'
    end
    object cdsTotalQuilo: TFloatField
      FieldName = 'Quilo'
    end
    object cdsTotalClassificacao: TFloatField
      FieldName = 'Classificacao'
    end
    object cdsTotalEsteira: TFloatField
      FieldName = 'Esteira'
    end
    object cdsTotalAjuste: TFloatField
      FieldName = 'Ajuste'
    end
    object cdsTotalNota: TFloatField
      FieldName = 'Nota'
    end
    object cdsTotalPedido: TFloatField
      FieldName = 'Pedido'
    end
    object cdsTotalAnterior: TFloatField
      FieldName = 'Anterior'
    end
  end
  object dsTotal: TDataSource
    DataSet = cdsTotal
    Left = 808
    Top = 368
  end
  object qrEstoque: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 535
    Top = 192
  end
  object qrProduto: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 599
    Top = 192
  end
  object TEstoque: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    SQL.Strings = (
      'SELECT * FROM ESTOQUEVER')
    Left = 663
    Top = 192
    object TEstoqueID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
    end
    object TEstoqueVCOD: TIntegerField
      FieldName = 'VCOD'
      Origin = 'VCOD'
    end
    object TEstoqueVDES: TStringField
      FieldName = 'VDES'
      Origin = 'VDES'
      Size = 30
    end
    object TEstoqueCCOD: TIntegerField
      FieldName = 'CCOD'
      Origin = 'CCOD'
    end
    object TEstoqueCDES: TStringField
      FieldName = 'CDES'
      Origin = 'CDES'
      Size = 30
    end
    object TEstoquePCOD: TIntegerField
      FieldName = 'PCOD'
      Origin = 'PCOD'
    end
    object TEstoquePDES: TStringField
      FieldName = 'PDES'
      Origin = 'PDES'
      Size = 50
    end
    object TEstoqueCALD: TStringField
      FieldName = 'CALD'
      Origin = 'CALD'
      Size = 15
    end
    object TEstoqueUNID: TStringField
      FieldName = 'UNID'
      Origin = 'UNID'
      Size = 5
    end
    object TEstoqueTEX1: TStringField
      FieldName = 'TEX1'
      Origin = 'TEX1'
      Size = 15
    end
    object TEstoqueENT1: TBCDField
      FieldName = 'ENT1'
      Origin = 'ENT1'
      Precision = 18
    end
    object TEstoqueSAI1: TBCDField
      FieldName = 'SAI1'
      Origin = 'SAI1'
      Precision = 18
    end
    object TEstoqueTEX2: TStringField
      FieldName = 'TEX2'
      Origin = 'TEX2'
      Size = 15
    end
    object TEstoqueENT2: TBCDField
      FieldName = 'ENT2'
      Origin = 'ENT2'
      Precision = 18
    end
    object TEstoqueSAI2: TBCDField
      FieldName = 'SAI2'
      Origin = 'SAI2'
      Precision = 18
    end
    object TEstoqueTEX3: TStringField
      FieldName = 'TEX3'
      Origin = 'TEX3'
      Size = 15
    end
    object TEstoqueENT3: TBCDField
      FieldName = 'ENT3'
      Origin = 'ENT3'
      Precision = 18
    end
    object TEstoqueSAI3: TBCDField
      FieldName = 'SAI3'
      Origin = 'SAI3'
      Precision = 18
    end
    object TEstoqueTEX4: TStringField
      FieldName = 'TEX4'
      Origin = 'TEX4'
      Size = 15
    end
    object TEstoqueENT4: TBCDField
      FieldName = 'ENT4'
      Origin = 'ENT4'
      Precision = 18
    end
    object TEstoqueSAI4: TBCDField
      FieldName = 'SAI4'
      Origin = 'SAI4'
      Precision = 18
    end
    object TEstoqueTEX5: TStringField
      FieldName = 'TEX5'
      Origin = 'TEX5'
      Size = 15
    end
    object TEstoqueENT5: TBCDField
      FieldName = 'ENT5'
      Origin = 'ENT5'
      Precision = 18
    end
    object TEstoqueSAI5: TBCDField
      FieldName = 'SAI5'
      Origin = 'SAI5'
      Precision = 18
    end
    object TEstoqueTEX6: TStringField
      FieldName = 'TEX6'
      Origin = 'TEX6'
      Size = 15
    end
    object TEstoqueENT6: TBCDField
      FieldName = 'ENT6'
      Origin = 'ENT6'
      Precision = 18
    end
    object TEstoqueSAI6: TBCDField
      FieldName = 'SAI6'
      Origin = 'SAI6'
      Precision = 18
    end
    object TEstoqueTEX7: TStringField
      FieldName = 'TEX7'
      Origin = 'TEX7'
      Size = 15
    end
    object TEstoqueENT7: TBCDField
      FieldName = 'ENT7'
      Origin = 'ENT7'
      Precision = 18
    end
    object TEstoqueSAI7: TBCDField
      FieldName = 'SAI7'
      Origin = 'SAI7'
      DisplayFormat = '#,0.00'
      Precision = 18
    end
    object TEstoqueTEX8: TStringField
      FieldName = 'TEX8'
      Origin = 'TEX8'
      Size = 15
    end
    object TEstoqueENT8: TBCDField
      FieldName = 'ENT8'
      Origin = 'ENT8'
      Precision = 18
    end
    object TEstoqueSAI8: TBCDField
      FieldName = 'SAI8'
      Origin = 'SAI8'
      DisplayFormat = '#,0.00'
      Precision = 18
    end
    object TEstoqueTEX9: TStringField
      FieldName = 'TEX9'
      Origin = 'TEX9'
      Size = 15
    end
    object TEstoqueENT9: TBCDField
      FieldName = 'ENT9'
      Origin = 'ENT9'
      Precision = 18
    end
    object TEstoqueSAI9: TBCDField
      FieldName = 'SAI9'
      Origin = 'SAI9'
      Precision = 18
    end
    object TEstoqueQANT: TBCDField
      FieldName = 'QANT'
      Origin = 'QANT'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQCLA: TBCDField
      FieldName = 'QCLA'
      Origin = 'QCLA'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQNOT: TBCDField
      FieldName = 'QNOT'
      Origin = 'QNOT'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQPED: TBCDField
      FieldName = 'QPED'
      Origin = 'QPED'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQAJU: TBCDField
      FieldName = 'QAJU'
      Origin = 'QAJU'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQEST: TBCDField
      FieldName = 'QEST'
      Origin = 'QEST'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueQATU: TBCDField
      FieldName = 'QATU'
      Origin = 'QATU'
      DisplayFormat = '0.###'
      Precision = 18
    end
    object TEstoqueACESOCOD: TIntegerField
      FieldName = 'ACESOCOD'
      Origin = 'ACESOCOD'
    end
    object TEstoqueREGLOG: TStringField
      FieldName = 'REGLOG'
      Origin = 'REGLOG'
      Size = 1000
    end
    object TEstoqueREG_ID: TIntegerField
      FieldName = 'REG_ID'
      Origin = 'REG_ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
  end
  object qrAnt_Ajuste: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 794
    Top = 137
  end
  object qrAnt_Entrada: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 874
    Top = 136
  end
  object qrAnt_Nota: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 946
    Top = 136
  end
  object qrAnt_Roman: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 1010
    Top = 136
  end
  object qrAnt_Pedido: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 1074
    Top = 136
  end
  object qrPedido: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 1074
    Top = 203
  end
  object qrRoman: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 1010
    Top = 204
  end
  object qrNota: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 946
    Top = 204
  end
  object qrEntrada: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 874
    Top = 204
  end
  object qrAjuste: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 794
    Top = 204
  end
  object qrRomanAberto: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 794
    Top = 268
  end
  object cdsEsteira: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 880
    Top = 266
    object cdsEsteiraCodigo: TIntegerField
      FieldName = 'Codigo'
    end
    object cdsEsteiraQtde: TFloatField
      FieldName = 'Qtde'
    end
  end
  object qrVariedade: TFDQuery
    CachedUpdates = True
    Connection = DM.BDConexao
    Transaction = DM.FDTransaction1
    UpdateOptions.AssignedValues = [uvAutoCommitUpdates]
    Left = 527
    Top = 248
  end
end
