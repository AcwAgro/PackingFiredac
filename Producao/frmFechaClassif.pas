...
type
  TFfrmFechaCassif = class(TFModelo)
    ...
    GroupBox4: TGroupBox;
    sbPesoCalculo: TSpeedButton;
    rzPeso: TRzNumericEdit;
    Label30: TLabel;
    DBEdit20: TDBEdit;
    cdsDadosxxTotalLiquidoMedia: TBCDField;
    cdsDadosRel: TClientDataSet;
    DBEdit100: TcxDBDateEdit;
    DBEdit32: TcxDBDateEdit;
    DBEdit31: TcxDBDateEdit;
    DBEdit312: TcxDBDateEdit;
    DBEdit311: TcxDBDateEdit;
    DBEdit314: TcxDBDateEdit;
    DBEdit313: TcxDBDateEdit;
    ...
    dbDadosDBTableView1xxTotalLiquidoMedia: TcxGridDBColumn;
    ...
    procedure sbPesoCalculoClick(Sender: TObject);
    procedure cdsDadosCalcFields(DataSet: TDataSet);
    procedure cdsitemCalcFields(DataSet: TDataSet);
  private
    { Private declarations }
    vComissaoTotal, vFunRural, vMediaSemSacolao, vExcluirClick: Boolean;
...
begin
  ...
  vMediaSemSacolao := (fBus(1,['Select valor From zGenerator Where Tabela = ''FECHA'' and Campo = ''MEDIASEMSACOLAO'''],1) = 1);
...
procedure TFfrmFechaCassif.cdsDadosCalcFields(DataSet: TDataSet);
begin
  inherited;
  if vMediaSemSacolao then
    cdsDadosxxTotalLiquidoMedia.asFloat := cdsDadosSACOLTOL.AsFloat
  else
    cdsDadosxxTotalLiquidoMedia.asFloat := cdsDadosSUBTOTAL.AsFloat;
end;
...
begin
  ...
  keyPreview := True;
  FMenu.ACBrEnterTab1.EnterAsTab := keyPreview;
  if (sbSalvar.Visible) and (not vExcluirClick)then
    fTotalizador(false);
end;
...
//    vlote := dm.qrSql5.FieldByName('Lanc').asInteger;
  ...
  vdatMed := '(Data>='+frInvD(cdsDadosDTPRECOINI.AsDateTime)+' and Data<'+frInvD(cdsDadosDTPRECOFIM.AsDateTime+1)+')';
...
{    if (Copy(cb1.Text,1,1) = '2') then
    ...
    begin
      ...
      begin
        ...
        begin
          vPesoProd := dm.qrSql3.FieldByName('pesoliq').asFloat;
          cdsItemUnid.asString  := dm.qrSql3.FieldByName('Unidade').asString;
          if (rzPeso.Value <> 0) and
             ((dm.qrSql3.FieldByName('pesoliq').asFloat > 17) and (dm.qrSql3.FieldByName('pesoliq').asFloat < 19))
          then
            cdsItemPeso.asFloat   := rzPeso.Value
          else
      ...
      begin
        ...
        begin
          if (rzPeso.Value <> 0) then
          begin
            cdsItemVlrMedia.asFloat  := frDeci(dm.qrSql1.FieldByName('Preco').asFloat / vPesoProd,4);
            cdsItemVlrCaixa.asFloat  := frDeci(cdsItemVlrMedia.asFloat * cdsItemPeso.asFloat,4);
          end
          else begin
...
begin
  ...
  vdat := '(Data>='+frInvD(cdsDadosDTFINANFIM.AsDateTime)+' and Data<'+frInvD(cdsDadosDTFINANFIM.AsDateTime+1)+')';
...
begin
  ...
  begin
    ...
    cdscnfREG_ID.AsInteger  := frGeneratorFB('GEN_FECHA_CLASSIF_CNF_ID');
    cdscnfLancit.asInteger   := vlanc;
    cdscnfData.AsDateTime         := dm.qrSql1.FieldByName('Data').AsDateTime;
...
begin
  ...
  sqlitem.SQL.Add('order by prodcod');
  cdsitem.Open;
  //Financeiro
  cdsfinan.close;
  sqlfinan.SQL.Clear;
...
var vreg: Integer;
    vcx, vkg, vemb, voper, vcom, vmed, vliq, vbins, vkgsaco, vcxsaco, vvlsaco: Double;
begin
...
{    if (Copy(cb1.Text,1,1) <> '1') then
    ...
    begin
      ...
      vvlsaco := vvlsaco + cdsitemVLRLIQUIDO.asFloat;
    end;
...
//    cdsItemVlrLiquido.asFloat := frDeci(cdsItemPreLiquido.asFloat * cdsItemQtdeKG.asFloat,2);
  ...
  cdsDadosSACOLTOL.asFloat     := cdsDadosSUBTOTAL.asFloat - vvlsaco;
...
var vi, vf: Integer;
    vp,vn: String;
    vpdf: String;
    vImprimeSacolao: Boolean;
begin
  inherited;

  vpdf := pPreparaPDF;

  vImprimeSacolao  := (fBus(1,['Select valor From zGenerator Where Tabela = ''FECHA'' and Campo = ''IMPRIMESACOLAO'''],1) = 1);
  ...
  begin
    ppLabel4.Caption  := 'Período '+DateToStr(cdsDadosDTLOTEINI.AsDateTime) +' à '+ DateToStr(cdsDadosDTLOTEFIM.AsDateTime);
    ...
    begin
      ...
      begin
        ...
        begin
          if cdsDados.FieldByName(vn).DataType in [ftBCD, ftFloat, ftCurrency] then
      ...
      begin
        ...
        begin
          ...
          begin
            if cdsItem.FieldByName(vn).DataType in [ftBCD, ftFloat, ftCurrency] then
  ...
  begin
    ppLabel102.Visible  := vImprimeSacolao;
    ppDBText47.Visible  := vImprimeSacolao;
    ppDBText3.Visible   := vImprimeSacolao;
    ppDBText40.Visible  := vImprimeSacolao;
    ppVariable16.Visible:= vImprimeSacolao;
    ppVariable10.Visible:= vImprimeSacolao;
    ppLine21.Visible    := vImprimeSacolao;
  ...
  begin
    ppLabel132.Visible  := vImprimeSacolao;
    ppDBText71.Visible  := vImprimeSacolao;
    ppDBText72.Visible  := vImprimeSacolao;
    ppDBText79.Visible  := vImprimeSacolao;
    ppVariable8.Visible := vImprimeSacolao;
    ppVariable9.Visible := vImprimeSacolao;
    ppLine30.Visible    := vImprimeSacolao;
...
begin
  ...
  begin
    ...
    if (vNFC = 30) then
    begin
      ppLabel141.Visible   := vImprimeSacolao;
      ppDBText69.Visible   := vImprimeSacolao;
      ppDBText70.Visible   := vImprimeSacolao;
      ppDBText93.Visible   := vImprimeSacolao;
      ppVariable19.Visible := vImprimeSacolao;
      ppVariable20.Visible := vImprimeSacolao;
      ppLine33.Visible     := vImprimeSacolao;
...
procedure TFfrmFechaCassif.sbPesoCalculoClick(Sender: TObject);
var vPesoProd: Double;
begin
  inherited;
  ...
  begin
    ...
    if (rzPeso.Value = 0) then
    begin
      fSql(1,3,['select pesoliq from cadprodu where codigo = 0' +cdsItemProdCod.asString],1);
      vPesoProd := dm.qrSql3.FieldByName('pesoliq').asFloat;
      cdsItemPeso.asFloat := vPesoProd
    end
    else begin
      if ((cdsItemPeso.asFloat > 17) and (cdsItemPeso.asFloat < 19)) then
        cdsItemPeso.asFloat := rzPeso.Value;
    end;
