...
type
  ...
  private
    ...
    procedure FResumo;
    procedure FImportar;
    procedure FImpProdutor;
  public
  end;

var
  FEstoqEntra: TFEstoqEntra;
  vkey,vtag:Integer;

implementation

uses Menu, Rotina, Data, Pesq, rlEstMerca, rlEstMercaRes, rlEstProdutor,
  rlEstMercaQtde;

{$R *.DFM}

procedure TFEstoqEntra.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FreeAndNil(FEstoqEntra);
end;

procedure TFEstoqEntra.FormCreate(Sender: TObject);
begin
...
procedure TFEstoqEntra.FormKeyPress(Sender: TObject; var Key: Char);
begin
  vKey:=0;
...
procedure TFEstoqEntra.FormActivate(Sender: TObject);
begin
...
procedure TFEstoqEntra.fMostra;
begin
  qrMerca.Close;
  qrMerca.Sql.Strings[1]:='From EstEntra';
...
procedure TFEstoqEntra.Novo1Click(Sender: TObject);
var vlan:Integer;
begin
  Case TComponent(Sender).Tag of
     1: begin
          fSql(4,1,['Select Max(Lanc) From EstEntra'],1);
          vLan:=dm.qrSql1.Fields[0].asInteger+1;
          TEstoque.Close;
          TEstoque.TableName:='EstEntra';
     ...
     3: begin
          if frExcl then begin
             fSql(4,1,['Delete From EstEntra',
                       'Where Lanc='+qrMerca.FieldByName('Lanc').Text],2);
             fMostra;
        end; end;
     6: begin fPqVariedade; xpqpesq:=-1; TPesq.Enabled:=True; end;
    ...
    15: begin POpcao.Visible:=True; PAno.Visible:=False; vtag:=5000; end;
    11: Table1.First;
    12: Table1.Prior;
    13: Table1.Next;
    14: Table1.Last;
...
procedure TFEstoqEntra.Table1AfterScroll(DataSet: TDataSet);
begin
...
procedure TFEstoqEntra.TPesqTimer(Sender: TObject);
begin
...
procedure TFEstoqEntra.DBGrid2KeyPress(Sender: TObject; var Key: Char);
begin
...
begin
  ...
  with qrEstMerca do begin
    ...
    qrProduto.Close;
    qrProduto.Sql.Strings[1]:='From EstEntra';
...
procedure TFEstoqEntra.fImpProdutor;
var vper,vpro,vdat:String;
begin
  vper := IntToStr(StrToIntDef(Trim(meCodigo.Text),0));
  if vper<>'0' then begin
     vpro:='Produto=0'+Table1.FieldByName('Codigo').Text+' and Codigo=0'+vper;
  end else begin
     vPro:='Produto=0'+Table1.FieldByName('Codigo').Text+' and (Codigo>=0 and Codigo<=9999999)';
  end;
  vDat :='(Data>='+frInvD(Ed4.Date)+' and Data<'+frInvD(Ed44.Date+1)+')';
  if qrEstProdutor<>Nil then qrEstProdutor.Destroy; Application.CreateForm(TqrEstProdutor,qrEstProdutor);
  with qrEstProdutor do begin
    qrLabel16.Caption:='Relação Inventário do Estoque Entrada por Produtor - Periodo '+DateToStr(Ed4.Date)+' a '+DateToStr(Ed44.Date);
    qrProduto.Close;
    qrProduto.Sql.Strings[1]:='From EstEntra';
    qrProduto.Sql.Strings[2]:='Where ('+vPro+') and '+vdat;
    qrProduto.Open;
...
procedure TFEstoqEntra.fResumo;
var vper,vpro,vdat:String;
begin
  vper := IntToStr(StrToIntDef(Trim(meCodigo.Text),0));
  if vper<>'0' then begin
...
procedure TFEstoqEntra.fReprocessa;
var vqtd,vtqt:Double; vtva,vval,vmed:Real; vCod,vLan:Integer; vper,vpro1,vPro,vdpro,vDat,vDat1,vLote:String;
begin
  vper := IntToStr(StrToIntDef(Trim(meCodigo.Text),0));
  if vper<>'0' then begin
     vpro1:='ee.Codigo=0'+vper;
     vpro:='VarieCod=0'+vper;
     vdpro:='Produto=0'+vper;
  end else begin
     vPro1:='(ee.Codigo>=0 and ee.Codigo<=9999999)';
     vPro:='(VarieCod>=0 and VarieCod<=9999999)';
     vdPro:='(Produto>=0 and Produto<=9999999)';
  end;
  vmed:=0;
  vDat :='(DataIni>='+frInvD(Ed4.Date)+' and DataIni<'+frInvD(Ed44.Date+1)+')';
  ...
  vLan:=dm.qrSql1.Fields[0].asInteger+1;
//Entrada
  fSql(2,1,['Select Data, Nota, ProdCod, vv.VarieCod, VarieQtd, ProdDes, vv.VarieDes, ee.Recibo',
            'From Entre ee, EntreVar vv',
            'Where (ee.Lanc=vv.Lanc) and '+vpro+' and '+vdat1],3);
  ...
  while not dm.qrSql1.eof do begin
    ...
    vlote := 'Rom.'+dm.qrSql1.Fields[7].asString;
//    if (Trim(dm.qrSql1.Fields[1].asString)<> '') then
//      vlote := vlote + '/NFP.'+dm.qrSql1.Fields[1].asString;
    vlote := Copy(vlote, 1,15);
    if dm.qrSql1.Fields[4].asFloat>0 then begin
       fSql(4,2,['Insert into EstEntra',
                 ' (Tipo,Lanc,Data,Doc,Codigo,Produto,QtdeEnt,Nome,ProdDes,Proc)',
                 ' Values ('+QuotedStr('E')+','+IntToStr(vlan)+
                 ','+frInvD(dm.qrSql1.Fields[0].Value)+
                 ', '+QuotedStr(vlote)+
                 ', '+dm.qrSql1.Fields[2].asString+
                 ', '+dm.qrSql1.Fields[3].asString+
                 ', '+frInvV(frDeci(dm.qrSql1.Fields[4].asFloat,3))+
...
//Saida
  fSql(2,1,['Select rr.Data, rr.Lote, ff.ProdCod as ProrCod, rr.VarieCod, rr.QtdeBin as QtdBin, ff.ProdDes as ProrDes, rr.VarieDes, ff.LoteEntra',
            'From Roman rr, RomanPro ff',
            'where (ff.Lanc = rr.Lanc) and '+vpro+' and '+vdat1],3);
  fBar(0,dm.qrSql1.RecordCount,'');
  while not dm.qrSql1.eof do begin
    vlan:=vlan+1;
    fBar(1,dm.qrSql1.RecordCount,dm.qrSql1.Fields[6].asString+'-'+dm.qrSql1.Fields[1].asString);
    vlote := 'Lote '+dm.qrSql1.Fields[1].asString;
    if (Trim(dm.qrSql1.Fields[7].asString)<> '') then
      vlote := vlote + '/Ent.'+dm.qrSql1.Fields[7].asString;
    vlote := Copy(vlote, 1,15);
    if dm.qrSql1.Fields[4].asFloat>0 then begin
       fSql(4,2,['Insert into EstEntra',
                 ' (Tipo,Lanc,Data,Doc,Codigo,Produto,QtdeSai,Nome,ProdDes,Proc)',
                 ' Values ('+QuotedStr('S')+','+IntToStr(vlan)+
                 ','+frInvD(dm.qrSql1.Fields[0].Value)+
                 ', '+QuotedStr(vlote)+
                 ', '+'0'+dm.qrSql1.Fields[2].asString+
                 ', '+dm.qrSql1.Fields[3].asString+
                 ', '+frInvV(frDeci(dm.qrSql1.Fields[4].asFloat,3))+
                 ', '+QuotedStr(dm.qrSql1.Fields[5].asString)+
                 ', '+QuotedStr(dm.qrSql1.Fields[6].asString)+
...
//Recalcula saldo
  ...
  if vper<>'T' then begin
    TEstoque.Filter:='Produto=0'+vper;
    TEstoque.Filtered:=True;
  end;
  TEstoque.Open;
  fSql(4,1,['Select * From EstEntra',
  ...
  fSql(4,2,['Select Max(Lanc) From EstEntra',
...
procedure TFEstoqEntra.SBB1Click(Sender: TObject);
begin
  TEstoque.Post;
...
procedure TFEstoqEntra.SBB2Click(Sender: TObject);
begin
  TEstoque.Close;
  PDados.Visible:=False;
  TEstoque.Close;
...
procedure TFEstoqEntra.DBEdit4Exit(Sender: TObject);
begin
  if TEstoque.FieldByName('Tipo').Text='E' then begin
...
procedure TFEstoqEntra.DBEdit7Exit(Sender: TObject);
begin
...
procedure TFEstoqEntra.fImportar;
