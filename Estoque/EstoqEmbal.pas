...
begin
  ...
  begin
    ...
    fSql(1,3,['Select Lanc,Qtde,Descricao,Codigo From CadProBx',
              'Where '+vpro2],2);
    fBar(0,dm.qrSql3.RecordCount,'Salvar Saída: '+Table1.FieldByName('Descricao').asString);
    while not dm.qrSql3.eof do
    begin
      fBar(1,1,'');

      if CCB20.Checked then //Pela Classificação
      begin
    ...
    //nome=                  ', '+QuotedStr(Copy(dm.qrSql1.Fields[4].asString,1,15)+'-'+dm.qrSql1.Fields[3].asString)+
      ...
      begin
        ...
        While not dm.qrSql1.eof do begin //Saída
